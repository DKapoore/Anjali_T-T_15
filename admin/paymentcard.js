...
async function ensureCardFontsLoaded_() {
  // Font loading is best-effort only. On an offline/PWA session some
  // browsers can leave document.fonts promises pending forever; that used
  // to leave the UI stuck on "Drawing payment card..." before canvas work.
  ...
  try { await Promise.race([loadFonts, timeout]); } catch (e) { /* fallback fonts are fine */ }
}
...
async function drawQrToCanvas_(text, size) {
  ...
  await new Promise((resolve, reject) => {
    ...
    timer = setTimeout(() => finish(new Error('QR_GENERATION_TIMEOUT')), 3000);
    try {
      QRCode.toCanvas(qrCanvas, text, { width: size, margin: 1, errorCorrectionLevel: 'M' }, finish);
