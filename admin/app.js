...
function renderPaymentCardResult_(bookingId, d, dataUrl) {
  ...
  if (dataUrl) {
    ...
    // phone's WhatsApp/share sheet, while Download saves the PNG directly.
    html += `<div class="payment-card-actions">
      <button class="btn btn-primary" id="btnShareCard">${t('share_card')}</button>
      <a class="btn btn-secondary" id="btnDownloadCard" download="${d.booking_id}-payment-card.png">${t('download_card')}</a>
    </div>`;
  ...
  saveAdminState({
    ...
    paymentCard: {
      ...
      cardOptions: d.__cardOptions || null, // full draw-args, for regenerating the image if dataUrl itself can't be restored (section 7)
  ...
  if (state.section === 'detail' && state.bookingId) {
    ...
    if (beyondPayment) {
      ...
      if (card.dataUrl) {
        ...
        // The rendered image wasn't available to restore (e.g. storage
        // limits) - regenerate the same card from the saved payload
        // instead of leaving it blank (section 7).
        try {
          const regenUrl = await generatePaymentCardDataUrl(card.cardOptions);
          renderPaymentCardResult_(card.bookingId, {
            booking_id: card.bookingId, upi_uri: card.upiUri, whatsapp_link: card.whatsappLink,
            advance_amount: card.advanceAmount, final_total: card.finalTotal, balance_amount: card.balanceAmount
          }, regenUrl);
        } catch (e) {
          console.error('Could not regenerate payment card from saved state:', e);
        }
      }
    }
    return true;
...
async function renderDetail() {
  ...
  if (allow.payment) $('btnGenPayment').addEventListener('click', () => generatePayment(b.booking_id, b));
...
async function generatePayment(bookingId, booking) {
  const area = $('cardPreviewArea');
  // The "Generate Payment Card" button is visible as soon as a booking is
  // opened - before any fare has ever been saved. Typing a number into the
  // Fare box does nothing until "Save Fare" is clicked, which is a very
  // easy mix-up since both buttons sit right next to each other. Catch it
  // here with a clear, specific message instead of letting the request
  // round-trip to the server first.
  const fareNotSaved = booking && (booking.advance_amount === '' || booking.advance_amount === undefined || booking.advance_amount === null);
  if (fareNotSaved) {
    area.innerHTML = '<p class="field-msg error">' + t('err_save_fare_first') + '</p>';
    return;
  }
  const lang = currentMsgLang_();
  const res = await callApiSafe('CREATE_PAYMENT_REQUEST', { booking_id: bookingId, lang: lang }, 'Generating payment request...');
  if (!res.success) { $('detailError').textContent = res.error.message; return; }
  const d = res.data;
  area.innerHTML = inlineLoaderHtml('Drawing payment card...');

  if (!SETTINGS_CACHE || !SETTINGS_CACHE.BUSINESS_NAME) {
    const sres = await callApiQuiet('GET_SETTINGS_ADMIN', {});
    if (sres.success) SETTINGS_CACHE = sres.data;
  }
  const s = SETTINGS_CACHE;

  const cardOptions = {
    businessName: s.BUSINESS_NAME || 'Anjali Tours & Travel',
    logoDataUrl: s.BUSINESS_LOGO_BASE64 || '',
    tagline: s.TAGLINE || '',
    cardLanguage: s.CARD_LANGUAGE || 'EN_HI',
    bookingId: d.booking_id,
    travelDateDisplay: d.travel_date_display || '',
    pickup: d.pickup || '',
    destination: d.destination || '',
    tripType: d.trip_type || '',
    totalFareDisplay: rupee(d.final_total),
    advanceDisplay: rupee(d.advance_amount),
    balanceDisplay: rupee(d.balance_amount),
    upiUri: d.upi_uri,
    whatsappNumberDisplay: (s.WHATSAPP_NUMBER || '').replace(/^91/, '')
  };

  let dataUrl = '';
  let drawError = '';
  try {
    // Final safety boundary: any unexpected browser/canvas implementation
    // issue must resolve to an error instead of leaving the loader forever.
    dataUrl = await Promise.race([
      generatePaymentCardDataUrl(cardOptions),
      new Promise((_, reject) => setTimeout(() => reject(new Error('CARD_GENERATION_TIMEOUT')), 10000))
    ]);
  } catch (e) {
    console.error('Payment card generation failed:', e);
    drawError = (e && e.message === 'QR_LIBRARY_NOT_LOADED')
      ? 'QR code generator failed to load. Please reload the page and try again.'
      : (e && e.message === 'QR_GENERATION_TIMEOUT')
        ? 'QR code generation timed out. Please check the UPI ID and try again.'
        : (e && e.message === 'CARD_GENERATION_TIMEOUT')
          ? 'Payment card generation timed out. Please reload and try again.'
          : 'Could not draw payment card image.';
  }

  if (drawError) {
    // A draw failure must be shown, never silently overwritten by the
    // WhatsApp-link button below (this used to happen - the error message
    // was set here and then immediately replaced).
    area.innerHTML = '<p class="field-msg error">' + drawError + '</p>'
      + (d.whatsapp_link ? `<a class="btn btn-primary" href="${d.whatsapp_link}" target="_blank" rel="noopener">${t('whatsapp_message_btn')}</a>` : '');
    return;
  }

  d.__cardOptions = cardOptions; // carried through so renderPaymentCardResult_ can save it for later regeneration
  renderPaymentCardResult_(bookingId, d, dataUrl);
}
...
async function shareCardImage_(dataUrl, bookingId, whatsappLink) {
  try {
    ...
    const file = new File([blob], bookingId + '-payment-card.png', { type: 'image/png' });
  ...
  a.href = dataUrl;
  a.download = bookingId + '-payment-card.png';
  a.click();
  if (whatsappLink) {
    window.open(whatsappLink, '_blank', 'noopener');
    alert(t('share_fallback_with_whatsapp'));
  } else {
    alert(t('share_fallback_no_whatsapp'));
  }
}
