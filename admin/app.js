...
function renderPaymentCardResult_(bookingId, d, dataUrl) {
  ...
  if (dataUrl) {
    ...
    // Share sits alone, right under the image, full-width and primary -
    // this is the button meant for sending the actual QR image to the
    // customer. Regenerate/Download are secondary, smaller, below it.
    html += `<button class="btn btn-primary" id="btnShareCard">${t('share_card')}</button>`;
    ...
    html += `<div class="inline-row">
      ...
      <a class="btn btn-secondary btn-inline" id="btnDownloadCard" download="${d.booking_id}-payment-card.png">${t('download_card')}</a>
    </div>`;
  } else if (!d.upi_uri) {
...
async function generatePayment(bookingId, booking) {
  ...
  let drawError = '';
  try {
    dataUrl = await generatePaymentCardDataUrl(cardOptions);
  } catch (e) {
